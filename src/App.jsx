...
const Row = ({ title, items, rowIndex, activeRow, activeCol, onMovieClick, onViewMore, rowRefMap }) => {
  ...
  useEffect(() => {
    if (activeRow === rowIndex && containerRef.current) {
      const activeElement = containerRef.current.children[activeCol];
      if (activeElement) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-[3vw]">
      ...
      <div className="relative w-full h-full bg-zinc-900/60 rounded-[3vw] overflow-hidden shadow-[0_0_5vw_rgba(0,0,0,0.5)] border border-white/10 animate-modal-pop backdrop-blur-3xl flex">
        ...
        <div ref={rightPanelRef} className="relative w-[45%] px-[4vw] pb-[4vw] pt-[8vw] flex flex-col justify-start z-10 overflow-y-auto">
          <button onClick={onClose} className={`absolute top-[3vw] right-[3vw] w-[4vw] h-[4vw] rounded-full flex items-center justify-center border transition-all ${focusClose ? 'bg-white text-black ring-2 ring-white/50 scale-110 border-white' : 'bg-white/5 hover:bg-white/20 border-white/10'}`}><X className={`w-[1.8vw] h-[1.8vw] ${focusClose ? 'text-black' : 'text-white'}`} /></button>
  ...
  return (
    <div className={`w-full min-h-screen flex flex-col items-center pt-[14vw] text-white transition-all duration-500 ${isPageActive ? 'opacity-100 blur-0' : 'opacity-40 blur-[2px]'}`}>
      ...
      <div className={`w-[50vw] h-[5vw] bg-zinc-900 rounded-full flex items-center px-[2vw] border transition-all duration-300 mb-[4vw] ${activeRow === -1 ? 'border-white scale-110 shadow-[0_0_2vw_rgba(255,255,255,0.2)] bg-zinc-800' : 'border-white/10 opacity-70'}`}>
        <span className={`text-[1.8vw] ${searchQuery ? 'text-white' : 'text-zinc-500'}`}>
           {searchQuery || 'พิมพ์ชื่อเรื่อง (ใช้คีย์บอร์ด)...'}
           {activeRow === -1 && <span className="animate-pulse ml-[0.1vw]">|</span>}
        </span>
      </div>
...
export default function App() {
  ...
  }, [activeRow, activeCol, lastActiveRow, lastActiveCol, selectedMovie, activeTab, categories, isGridMode, isSearchMode, gridMovies, gridColumns, currentGridRows, currentFilter, gridPage, paginationItems, searchQuery, currentMoviesOnPage, heroMovie]);
  ...
  // Push a history state when opening Modal so TV hardware back can close it
  useEffect(() => {
    if (selectedMovie) {
  ...
  // Handle browser/TV back to close Modal at App level
  useEffect(() => {
    const onPop = () => {
      if (selectedMovie) {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      const { isSearchMode, activeRow, searchQuery } = stateRef.current;
      if (isSearchMode && activeRow === -1) {
      ...
      const {
        ...
        categories, isGridMode: currentIsGridMode, gridMovies, gridColumns, currentGridRows, 
        NAV_ITEMS, currentFilter, gridPage, paginationItems, currentMoviesOnPage
      } = stateRef.current;

      // Map TV hardware back keys to the same behavior as Escape/Backspace
      const isTvBack = (e.key === 'Back' || e.code === 'Back' || e.key === 'GoBack' || e.keyCode === 10009 || e.keyCode === 461);
      if (isTvBack) {
        e.preventDefault();
      ...
      switch (e.key) {
        ...
        case 'ArrowDown':
          if (currentActiveRow === -2) { 
             const nextRow = lastActiveRow !== -1 ? lastActiveRow : -1;
             if (activeTab === 'account') return;
             
             if (lastActiveRow !== null && lastActiveRow !== -1) {
                 setActiveRow(lastActiveRow);
                 setActiveCol(lastActiveCol || 0);
             } else {
                 if (['y_content', 'k_content', 'festival'].includes(activeTab)) {
                     if (currentMoviesOnPage.length > 0) {
                        setActiveRow(-1); setActiveCol(0);
                     }
                 } else if (isSearchMode) { setActiveRow(-1); setActiveCol(0); }
                 else if (currentIsGridMode) { setActiveRow(-1); setActiveCol(0); } 
                 else { setActiveRow(-1); setActiveCol(0); }
             }
          } else {
             if (currentIsGridMode || isSearchMode) {
               if (currentActiveRow === -1) { 
                  if (currentGridRows > 0) {
                   ...
                   setActiveRow(prev => prev + 1);
               } else if (currentIsGridMode && currentActiveRow === currentGridRows - 1 && paginationItems.length > 0) {
                   let target = 0;
        ...
        case 'ArrowRight':
           if (currentActiveRow === -2) {
             ...
             if (currentActiveRow === -1) setActiveCol(prev => Math.min(1, prev + 1));
                 ...
                 const maxCol = Math.min(categories[catIdx].items.length, 12); 
                 setActiveCol(prev => Math.min(maxCol, prev + 1));
             }
           }
           break;
        case 'Enter':
          if (currentActiveRow === -2) {
             ...
             if (item) {
                 ...
                 else if (item.type === 'next') newPage = Math.min(totalPages, gridPage + 1);
             ...
             if (currentIsGridMode || isSearchMode) {
               ...
               if (currentActiveRow === -1) { 
                 ...
                 if (col in map) {
                   ...
                   const catIdx = currentActiveRow - 1;
                   const limited = categories[catIdx].items.slice(0, 12);
  ...
  const renderContent = () => {
    ...
    return (
      <main className="relative z-10">
        <Hero movie={heroMovie} isFocused={activeRow === -1} activeBtnIndex={activeCol} onPlay={() => setSelectedMovie(heroMovie)} />
